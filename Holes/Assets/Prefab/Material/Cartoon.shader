Shader "Unlit/Test3"
{
    Properties
    {
        ...
        _OutlineWidth("Outline Width", Range(0.01, 2)) = 0.01
        _OutLineColor("OutLine Color", Color) = (0.5,0.5,0.5,1)

        _RampStart("RampStart", Range(0.1, 1)) = 0.3//交界起始
        _RampSize("RampSize", Range(0, 1)) = 0.1//交界大小
        [IntRange] _RampStep("RampStep", Range(1,10)) = 1//交界段数
        _RampSmooth("RampSmooth", Range(0.01, 1)) = 0.1//交界柔和度
        _DarkColor("DarkColor", Color) = (0.4, 0.4, 0.4, 1)//暗面
        _LightColor("LightColor", Color) = (0.8, 0.8, 0.8, 1)//亮面

        _SpecPow("SpecPow", Range(0, 1)) = 0.1//光泽度
        _SpecularColor("SpecularColor", Color) = (1.0, 1.0, 1.0, 1)//高光
        _SpecIntensity("SpecIntensity", Range(0, 1)) = 0//高光强度
        _SpecSmooth("SpecSmooth", Range(0, 0.5)) = 0.1//高光柔和度

        _RimColor("RimColor", Color) = (1.0, 1.0, 1.0, 1)//边缘光
        _RimThreshold("RimThreshold", Range(0, 1)) = 0.45//边缘光阈值
        _RimSmooth("RimSmooth", Range(0, 0.5)) = 0.1//边缘光柔和度
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                v2f o;
                float4 newVertex = float4(v.vertex.xyz * (1.0f + _OutlineWidth), 1);//* _OutlineWidth,1);////+ normalize(v.normal) * _OutlineWidth * 0.05,1);
